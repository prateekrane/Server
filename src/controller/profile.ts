...
export const getPlaylistAudios: RequestHandler = async (req, res) => {
  ...
  const [playlistResult] = await Playlist.aggregate(aggregataionLogic);

  if (!playlistResult) {
    const [autoPlaylistResult] = await Playlist.aggregate(aggregataionLogic);
